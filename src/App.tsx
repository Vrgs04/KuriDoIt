...
function Admin() {
  type AdminMarket = { id: string; match_id: string; market_type: string; title: string; line: number | null; status: string; option_id?: string; option_label?: string; line_value?: number | null; decimal_odds?: number }
  type RankingUser = { id: string; name: string; points: number; won: number; lost: number; pending: number; total: number }
  const nav = useNavigate(), [stats, setStats] = useState<Record<string, number>>(), [matches, setMatches] = useState<Match[]>([]), [picks, setPicks] = useState<Array<Pick & { name: string }>>([]), [ranking, setRanking] = useState<RankingUser[]>([]), [markets, setMarkets] = useState<AdminMarket[]>([]), [selectedMatch, setSelectedMatch] = useState(''), [selectedMarket, setSelectedMarket] = useState(''), [deleteMatchId, setDeleteMatchId] = useState(''), [moderateUserId, setModerateUserId] = useState(''), [error, setError] = useState(''), [notice, setNotice] = useState('')
  async function load() { try { const [nextStats, nextMatches, nextPicks, nextRanking] = await Promise.all([api<Record<string, number>>('/admin/dashboard'), api<Match[]>('/matches'), api<Array<Pick & { name: string }>>('/admin/picks'), api<RankingUser[]>('/leaderboard')]); setStats(nextStats); setMatches(nextMatches); setPicks(nextPicks); setRanking(nextRanking) } catch (e) { if ((e as Error).message === 'No autorizado') nav('/admin/login'); else setError((e as Error).message) } }
  async function loadMarkets(matchId: string) { setSelectedMatch(matchId); setSelectedMarket(''); setMarkets(matchId ? await api<AdminMarket[]>(`/admin/markets?match_id=${encodeURIComponent(matchId)}`) : []) }
  useEffect(() => { load() }, [])
  async function addMatch(e: FormEvent<HTMLFormElement>) { e.preventDefault(); const form=e.currentTarget, f = new FormData(form); setError(''); try { const created=await api<{id:string}>('/admin/matches', { method: 'POST', body: JSON.stringify({ opponent: f.get('opponent'), kuriyama_side: f.get('side'), kickoff_at: new Date(String(f.get('kickoff'))).toISOString(), picks_close_at: new Date(String(f.get('close'))).toISOString(), status: 'OPEN' }) }); form.reset(); await load(); await loadMarkets(created.id); setNotice('Partido creado correctamente') } catch (x) { setError((x as Error).message) } }
  async function addMarket(e: FormEvent<HTMLFormElement>) { e.preventDefault(); const form=e.currentTarget, f=new FormData(form); setError(''); try { const created=await api<{id:string}>('/admin/markets',{method:'POST',body:JSON.stringify({match_id:selectedMatch,market_type:f.get('type'),title:f.get('title'),line:f.get('line')?Number(f.get('line')):null,status:'OPEN'})}); form.reset(); await loadMarkets(selectedMatch); setSelectedMarket(created.id); setNotice('Mercado creado correctamente') } catch(x){setError((x as Error).message)} }
  async function addOption(e: FormEvent<HTMLFormElement>) { e.preventDefault(); const form=e.currentTarget, f=new FormData(form); setError(''); try { await api('/admin/options',{method:'POST',body:JSON.stringify({market_id:selectedMarket,label:f.get('label'),line_value:f.get('line')?Number(f.get('line')):null,decimal_odds:Number(f.get('odds'))})}); form.reset(); await loadMarkets(selectedMatch); setNotice('Momio agregado correctamente') } catch(x){setError((x as Error).message)} }
  async function removeMatch(id:string){try{await api(`/admin/matches/${id}`,{method:'DELETE'});setDeleteMatchId('');if(selectedMatch===id){setSelectedMatch('');setMarkets([])}await load();setNotice('Partido eliminado')}catch(x){setError((x as Error).message)}}
  async function moderateUser(id:string){try{await api(`/admin/users/${id}`,{method:'DELETE'});setModerateUserId('');await load();setNotice('Usuario retirado del ranking')}catch(x){setError((x as Error).message)}}
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><div className="wordmark"><b>KURI</b><span>DOIT</span></div><small>ADMIN</small><nav><a className="active">Dashboard</a><a href="#admin-matches">Partidos</a><a href="#admin-markets">Mercados</a><a href="#admin-picks">Picks</a><a href="#admin-ranking">Ranking</a></nav><button onClick={logout}><LogOut/> Cerrar sesión</button></aside><main className="admin-main"><div className="admin-mobile-head"><div className="wordmark"><b>KURI</b><span>DOIT</span></div><button onClick={logout}><LogOut/></button></div><span className="section-kicker">CENTRO DE CONTROL</span><h1>Dashboard</h1>{error && <p className="error">{error}</p>}{notice&&<div className="admin-notice"><Check/>{notice}<button onClick={()=>setNotice('')}><X/></button></div>}<div className="stats">{['users', 'picks', 'pending', 'won', 'lost'].map(k => <article key={k}><span>{({ users: 'Usuarios', picks: 'Picks', pending: 'Pendientes', won: 'Ganados', lost: 'Perdidos' } as Record<string, string>)[k]}</span><b>{stats?.[k] ?? '—'}</b></article>)}</div><section className="admin-card" id="admin-matches"><h2>Nuevo partido</h2><form className="admin-form" onSubmit={addMatch}><label>Rival<input name="opponent" required minLength={2}/></label><label>Sede<select name="side"><option value="HOME">Local</option><option value="AWAY">Visitante</option></select></label><label>Inicio<input name="kickoff" type="datetime-local" required/></label><label>Cierre picks<input name="close" type="datetime-local" required/></label><button className="primary-button">CREAR PARTIDO</button></form></section><section className="admin-card"><h2>Partidos</h2><div className="admin-table match-admin-table"><div className="table-head"><span>Partido</span><span>Fecha</span><span>Estado</span><span>Acciones</span></div>{matches.map(m => <div key={m.id}><b>Kuriyama vs {m.opponent}</b><span>{date(m.kickoff_at)}</span><StatusBadge status={m.status}/><div className="row-actions"><button onClick={()=>loadMarkets(m.id)}>Momios</button>{deleteMatchId===m.id?<><button onClick={()=>setDeleteMatchId('')}>Cancelar</button><button className="danger-small" onClick={()=>removeMatch(m.id)}>Confirmar</button></>:<button className="danger-small" onClick={()=>setDeleteMatchId(m.id)}><Trash2/>Eliminar</button>}</div></div>)}</div></section><section className="admin-card" id="admin-markets"><h2>Mercados y momios</h2><label className="admin-select">Partido<select value={selectedMatch} onChange={e=>loadMarkets(e.target.value)}><option value="">Selecciona un partido</option>{matches.map(m=><option key={m.id} value={m.id}>Kuriyama vs {m.opponent} · {date(m.kickoff_at)}</option>)}</select></label>{selectedMatch&&<div className="market-admin-grid"><form className="admin-form compact-form" onSubmit={addMarket}><h3>1. Crear mercado</h3><label>Tipo<select name="type"><option value="MONEYLINE">Ganador</option><option value="BOTH_SCORE">Ambos anotan</option><option value="TOTAL_GOALS">Total de goles</option><option value="ASIAN_HANDICAP">Handicap asiático</option><option value="TOTAL_CORNERS">Total de corners</option></select></label><label>Título<input name="title" placeholder="Ej. Total de goles 2.5" required/></label><label>Línea opcional<input name="line" type="number" step="0.5"/></label><button className="primary-button">CREAR MERCADO</button></form><form className="admin-form compact-form" onSubmit={addOption}><h3>2. Agregar selección y momio</h3><label>Mercado<select value={selectedMarket} onChange={e=>setSelectedMarket(e.target.value)} required><option value="">Selecciona un mercado</option>{uniqueMarkets.map(m=><option key={m.id} value={m.id}>{m.title}</option>)}</select></label><label>Selección<input name="label" placeholder="Ej. Over 2.5" required/></label><label>Línea opcional<input name="line" type="number" step="0.5"/></label><label>Momio decimal<input name="odds" type="number" min="1" max="1000" step="0.01" placeholder="1.85" required/></label><button className="primary-button" disabled={!selectedMarket}>AGREGAR MOMIO</button></form></div>}{selectedMatch&&<div className="configured-markets">{uniqueMarkets.length?uniqueMarkets.map(m=><article key={m.id}><b>{m.title}</b><small>{m.market_type}</small><div>{markets.filter(o=>o.id===m.id&&o.option_id).map(o=><span key={o.option_id}>{o.option_label}<strong>{fmt(o.decimal_odds!)}</strong></span>)}</div></article>):<div className="empty-state">Este partido todavía no tiene mercados.</div>}</div>}</section><section className="admin-card" id="admin-picks"><h2>Últimos picks</h2><div className="admin-table picks-table"><div className="table-head"><span>Usuario</span><span>Mercado / Pick</span><span>Momio</span><span>Estado</span></div>{picks.slice(0, 20).map(p => <div key={p.id}><b>{p.name}</b><span>{p.market_title} · {p.option_label}</span><strong>{fmt(p.odds_snapshot)}</strong><StatusBadge status={p.status}/></div>)}</div></section><section className="admin-card" id="admin-ranking"><h2>Moderación del ranking</h2><p className="admin-help">Retira nombres ofensivos. Los picks históricos permanecen anonimizados.</p><div className="admin-table moderation-table"><div className="table-head"><span>Usuario</span><span>Puntos</span><span>Picks</span><span>Acción</span></div>{ranking.map(u=><div key={u.id}><b>{u.name}</b><span>{fmt(u.points)}</span><span>{u.total}</span><div className="row-actions">{moderateUserId===u.id?<><button onClick={()=>setModerateUserId('')}>Cancelar</button><button className="danger-small" onClick={()=>moderateUser(u.id)}>Confirmar</button></>:<button className="danger-small" onClick={()=>setModerateUserId(u.id)}><Trash2/>Retirar</button>}</div></div>)}</div></section></main></div>
}
