...
const questionType = z.enum(['CUSTOM','TOTAL_GOALS','FIRST_HALF_GOALS','GOAL_SCORER'])
...
const questionSchema = z.object({ match_id: z.string().uuid(), prompt: z.string().trim().min(3).max(240), points_value: z.number().positive().max(100), status: z.enum(['OPEN','CLOSED','SETTLED','DISABLED']).default('OPEN'), question_type: questionType.default('CUSTOM'), options: z.array(questionOptionSchema).min(2).max(60) })

async function questionsWithOptions(db: D1Database, matchId: string, userId?: string) {
  const sql='SELECT q.id,q.match_id,q.prompt,q.points_value,q.status,q.question_type,q.correct_answer,q.settled_at,m.kickoff_at,m.picks_close_at FROM questions q JOIN matches m ON m.id=q.match_id WHERE q.match_id=? AND q.status!=\'DISABLED\' ORDER BY q.created_at'
...
async function route(c: Ctx) {
  ...
  if(method==='GET'&&userPredictions){const own=await userToken(c.request,db,userPredictions[1]);const visibility=own?'':"AND q.status='SETTLED'";return json((await db.prepare(`SELECT p.*,q.prompt,q.points_value,m.opponent,m.kickoff_at,COALESCE(o.label,p.answer) answer_label FROM predictions p JOIN questions q ON q.id=p.question_id JOIN matches m ON m.id=p.match_id LEFT JOIN question_options o ON o.question_id=p.question_id AND o.value_key=p.answer WHERE p.user_id=? ${visibility} ORDER BY m.kickoff_at DESC,q.created_at`).bind(userPredictions[1]).all()).results)}
  ...
  if(method==='PUT'&&matchResult){const v=await body(c.request,z.object({kuriyama_score:z.number().int().min(0).max(99).nullable(),opponent_score:z.number().int().min(0).max(99).nullable(),status:z.enum(['DRAFT','OPEN','LOCKED','FINISHED','CANCELLED'])}).refine(value=>value.status!=='FINISHED'||(value.kuriyama_score!==null&&value.opponent_score!==null),'Un partido finalizado necesita marcador'));await db.prepare('UPDATE matches SET kuriyama_score=?,opponent_score=?,status=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(v.kuriyama_score,v.opponent_score,v.status,matchResult[1]).run();return json({ok:true})}
  ...
  if(method==='GET'&&path==='/admin/question-templates'){const questions=(await db.prepare('SELECT q.id,q.match_id,q.prompt,q.points_value,q.question_type,m.opponent,m.kickoff_at FROM questions q JOIN matches m ON m.id=q.match_id ORDER BY q.created_at DESC LIMIT 200').all()).results as Array<Record<string,unknown>>;const options=(await db.prepare('SELECT qo.* FROM question_options qo JOIN questions q ON q.id=qo.question_id ORDER BY qo.question_id,qo.sort_order').all()).results as Array<Record<string,unknown>>;return json(questions.map(q=>({...q,options:options.filter(o=>o.question_id===q.id)})))}
  if(method==='POST'&&path==='/admin/questions/copy'){const v=await body(c.request,z.object({source_question_id:z.string().uuid(),match_id:z.string().uuid()}));const source=await db.prepare('SELECT prompt,points_value,question_type FROM questions WHERE id=?').bind(v.source_question_id).first<Record<string,string|number>>();if(!source)return json({error:'Pregunta original no encontrada'},404);const options=(await db.prepare('SELECT value_key,label,points_value FROM question_options WHERE question_id=? ORDER BY sort_order').bind(v.source_question_id).all()).results as Array<{value_key:string,label:string,points_value:number}>;if(options.length<2)return json({error:'La pregunta original no tiene respuestas suficientes'},409);const key=id();await db.prepare("INSERT INTO questions(id,match_id,prompt,points_value,status,question_type) VALUES(?,?,?,?, 'OPEN',?)").bind(key,v.match_id,source.prompt,source.points_value,source.question_type).run();await replaceQuestionOptions(db,key,options);return json({id:key},201)}
  if(method==='POST'&&path==='/admin/questions'){const v=await body(c.request,questionSchema),key=id();await db.prepare('INSERT INTO questions(id,match_id,prompt,points_value,status,question_type) VALUES(?,?,?,?,?,?)').bind(key,v.match_id,v.prompt,v.points_value,v.status,v.question_type).run();await replaceQuestionOptions(db,key,v.options);return json({id:key},201)}
  const adminQuestion=path.match(/^\/admin\/questions\/([^/]+)$/)
  if(method==='PUT'&&adminQuestion){const v=await body(c.request,questionSchema);const current=await db.prepare('SELECT status FROM questions WHERE id=?').bind(adminQuestion[1]).first<{status:string}>();if(!current)return json({error:'Pregunta inválida'},404);if(current.status==='SETTLED')return json({error:'Una pregunta resuelta no se puede modificar'},409);await db.prepare("UPDATE questions SET prompt=?,points_value=?,status=?,question_type=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(v.prompt,v.points_value,v.status,v.question_type,adminQuestion[1]).run();await replaceQuestionOptions(db,adminQuestion[1],v.options);return json({ok:true})}
  ...
  if(method==='POST'&&path==='/admin/questions/settle'){
    const v=await body(c.request,z.object({question_id:z.string().uuid(),correct_answers:z.array(z.string().min(1).max(100)).max(60).default([]),numeric_result:z.number().min(0).max(100).optional(),void:z.boolean().default(false)}))
    const question=await db.prepare('SELECT question_type FROM questions WHERE id=?').bind(v.question_id).first<{question_type:string}>();if(!question)return json({error:'Pregunta inválida'},404)
