...
const questionSchema = z.object({ match_id: z.string().uuid(), prompt: z.string().trim().min(5).max(240), points_value: z.number().positive().max(100), status: z.enum(['OPEN','CLOSED','SETTLED','DISABLED']).default('OPEN') })
...
async function route(c: Ctx) {
  ...
  if(method==='GET'&&matchQuestions){const userId=url.searchParams.get('user_id');if(userId&&!(await userToken(c.request,db,userId)))return json({error:'Sesión de usuario inválida'},401);const sql=`SELECT q.id,q.match_id,q.prompt,q.points_value,q.status,m.kickoff_at,m.picks_close_at${userId?',p.answer prediction_answer,p.status prediction_status,p.points_awarded':''} FROM questions q JOIN matches m ON m.id=q.match_id ${userId?'LEFT JOIN predictions p ON p.question_id=q.id AND p.user_id=?':''} WHERE q.match_id=? AND q.status!='DISABLED' ORDER BY q.created_at`;return json((await db.prepare(sql).bind(...(userId?[userId,matchQuestions[1]]:[matchQuestions[1]])).all()).results)}
  if(method==='POST'&&path==='/predictions'){const v=await body(c.request,z.object({user_id:z.string().uuid(),question_id:z.string().uuid(),answer:z.enum(['YES','NO'])}));if(!(await userToken(c.request,db,v.user_id)))return json({error:'Sesión de usuario inválida'},401);const q=await db.prepare("SELECT q.id,q.match_id,q.points_value,q.status,m.status match_status,m.kickoff_at,m.picks_close_at FROM questions q JOIN matches m ON m.id=q.match_id WHERE q.id=?").bind(v.question_id).first<Record<string,string|number>>();if(!q)return json({error:'Pregunta inválida'},404);if(q.status!=='OPEN'||q.match_status!=='OPEN'||Date.now()>=new Date(String(q.kickoff_at)).getTime()||Date.now()>=new Date(String(q.picks_close_at)).getTime())return json({error:'Las predicciones están cerradas'},409);const predictionId=id();await db.prepare("INSERT INTO predictions(id,user_id,match_id,question_id,answer,points_snapshot) VALUES(?,?,?,?,?,?) ON CONFLICT(user_id,question_id) DO UPDATE SET answer=excluded.answer,points_snapshot=excluded.points_snapshot,updated_at=CURRENT_TIMESTAMP WHERE status='PENDING'").bind(predictionId,v.user_id,q.match_id,v.question_id,v.answer,q.points_value).run();return json({ok:true},201)}
  const userPredictions=path.match(/^\/users\/([^/]+)\/predictions$/)
  if(method==='GET'&&userPredictions){const own=await userToken(c.request,db,userPredictions[1]);const visibility=own?'':'AND datetime(m.kickoff_at,\'-30 minutes\') <= CURRENT_TIMESTAMP';return json((await db.prepare(`SELECT p.*,q.prompt,q.points_value,m.opponent,m.kickoff_at FROM predictions p JOIN questions q ON q.id=p.question_id JOIN matches m ON m.id=p.match_id WHERE p.user_id=? ${visibility} ORDER BY m.kickoff_at DESC,q.created_at`).bind(userPredictions[1]).all()).results)}
  ...
  if(method==='GET'&&path==='/admin/questions'){const matchId=url.searchParams.get('match_id');if(!matchId)return json({error:'Falta match_id'},400);return json((await db.prepare('SELECT * FROM questions WHERE match_id=? ORDER BY created_at').bind(matchId).all()).results)}
  if(method==='POST'&&path==='/admin/questions'){const v=await body(c.request,questionSchema),key=id();await db.prepare('INSERT INTO questions(id,match_id,prompt,points_value,status) VALUES(?,?,?,?,?)').bind(key,v.match_id,v.prompt,v.points_value,v.status).run();return json({id:key},201)}
  const adminQuestion=path.match(/^\/admin\/questions\/([^/]+)$/)
  if(method==='PUT'&&adminQuestion){const v=await body(c.request,questionSchema);await db.prepare("UPDATE questions SET prompt=?,points_value=?,status=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND status!='SETTLED'").bind(v.prompt,v.points_value,v.status,adminQuestion[1]).run();return json({ok:true})}
  ...
  if(method==='POST'&&path==='/admin/questions/settle'){const v=await body(c.request,z.object({question_id:z.string().uuid(),correct_answer:z.enum(['YES','NO','VOID'])}));const question=await db.prepare('SELECT points_value FROM questions WHERE id=?').bind(v.question_id).first();if(!question)return json({error:'Pregunta inválida'},404);await db.batch([db.prepare("UPDATE questions SET status='SETTLED',correct_answer=?,settled_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(v.correct_answer,v.question_id),db.prepare("UPDATE predictions SET status=CASE WHEN ?='VOID' THEN 'VOID' WHEN answer=? THEN 'CORRECT' ELSE 'INCORRECT' END,points_awarded=CASE WHEN ?='VOID' THEN 0 WHEN answer=? THEN points_snapshot ELSE -points_snapshot END,settled_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE question_id=?").bind(v.correct_answer,v.correct_answer,v.correct_answer,v.correct_answer,v.question_id)]);return json({ok:true})}
  if(method==='GET'&&path==='/admin/predictions'){return json((await db.prepare('SELECT p.*,u.name,q.prompt,m.opponent,m.kickoff_at FROM predictions p JOIN users u ON u.id=p.user_id JOIN questions q ON q.id=p.question_id JOIN matches m ON m.id=p.match_id ORDER BY p.created_at DESC LIMIT 500').all()).results)}
